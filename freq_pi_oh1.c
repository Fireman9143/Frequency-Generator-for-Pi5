...
#define PLL0_FREQUENCY 1000000000.0

//Variables used by auto Pi model detection
static volatile uint32_t piModel = 1;
static volatile uint32_t piPeriphBase = 0x40000000;
...
#define CM_GP0CTL (0x4001070)
#define GPFSEL0 (0x400d0000)
#define CM_GP0DIV (0x4001074)
#define CLKBASE (0x40010000)
#define DMABASE (0x40007000)
#define PWMBASE  (0x40020000) /* PWM controller  */
...
/* open /dev/gpiomem4 */
if ((mem_fd = open(gpiomem_path, O_RDWR|O_SYNC) ) < 0)
	{
	perror("can't open /dev/gpiomem4 \n");
	exit(EXIT_FAILURE);
   }
...
{
/* open /dev/mem */
if( (mem_fd = open("/dev/gpiomem4", O_RDWR|O_SYNC) ) < 0)
	{
	fprintf(stderr, "freq_pi: can't open /dev/gpiomem4, aborting.\n");
...
fprintf(stderr,\
"\nPanteltje freq_pi-%s\n\
Usage:\nfreq_pi [-b begin_frequency] [-d step_delay] [-e end_frequency] [-f frequency] [-h] [-i frequency_increment] [-l] [-r] [-s scan_delay] [-q] [-v] [-w] [-y ppm_correction]\n\
...
if(end_frequency <= begin_frequency)
	...
	while(n < 20000)
		{
		set_frequency(begin_frequency);
		usleep(500);
		set_frequency(end_frequency);
		usleep(500);
		n = n + 1;
		}		
...
while(1)
	...
	if(wait_for_gpio8_flag)
		...
		while(1)
			...
			usleep(1000); // 1 ms reduce procesor cycles
			}
